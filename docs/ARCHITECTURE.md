...
> [!NOTE]
> **Dependency Notation**: In the Mermaid diagram below, `A --> B` denotes **"A depends on B"** (matching `npx nx graph`).

```mermaid
graph TD
    common-logging --> common-core
    domain-model --> common-core
    persistence --> domain-model

    order-service --> common-core
    order-service --> common-logging
    order-service --> domain-model
    order-service --> persistence

    inventory-service --> common-core
    inventory-service --> common-logging

    coverage-aggregate --> common-core
    coverage-aggregate --> common-logging
    coverage-aggregate --> domain-model
    coverage-aggregate --> persistence
    coverage-aggregate --> order-service
    coverage-aggregate --> inventory-service
```
